# The effects of participation in cash transfer programs on political support
# for the government Aydin, Salvi and Solidoro
...
gov_transfers |>
  ...
  geom_point(color = "darkcyan", position = position_jitter(
    width = 0,
    height = 0.12
  )) +
  theme_light()
...
full_graph_q <- gov_transfers |>
  ...
  geom_smooth(method = "lm", formula = y ~
    poly(x, degree = 2), color = "navy", fill = "lightblue") +
  theme_light()
full_graph_q
...
full_graph_c <- gov_transfers |>
  ...
  geom_vline(xintercept = 0, color = "gray") +
  geom_smooth(method = "lm", formula = y ~
    poly(x, degree = 3), color = "navy", fill = "lightblue") +
  theme_light()
full_graph_c
...
grid.arrange(grobs = list(
  full_graph, full_graph_q, full_graph_c, half_graph, half_graph_q,
  half_graph_c, quart_graph, quart_graph_q, quart_graph_c
), ncol = 3)
...
full_slope <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered,
  data = gov_transfers
)
full_slope |>
  summary()

half_slope <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered,
  data = half_bw
)
half_slope |>
  summary()

quart_slope <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered,
  data = quart_bw
)
quart_slope |>
  summary()

full_age <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered +
  Age, data = gov_transfers)
full_age |>
  summary()

half_age <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered +
  Age, data = gov_transfers)
half_age |>
  summary()

quart_age <- lm(Support ~ Participation + Income_Centered + Participation * Income_Centered +
  Age, data = half_bw)
quart_age |>
  summary()

full_specs <- modelsummary(list(full_lm, full_q, full_c, full_slope, full_age), stars = T)
full_specs

half_specs <- modelsummary(list(half_lm, half_q, half_c, half_slope, half_age), stars = T)
half_specs

quart_specs <- modelsummary(list(quart_lm, quart_q, quart_c, quart_slope, quart_age),
  stars = T
)
quart_specs
